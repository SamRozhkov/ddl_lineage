...
function DatabaseConnectionForm({ onDdlExtracted }) {
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => ({
      ...
      ...(name === 'type' && {
        ...
        schema: value === 'postgresql' ? 'public' : ''
      })
    }));
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium mb-1">
            Objects (optional, comma-separated)
          </label>
          <input
            type="text"
            name="objects"
            ...
            onChange={handleChange}
            className="w-full p-2 border rounded"
            placeholder="table1, table2, view1"
          />
        </div>
        {error && (
        ...
        >
          {loading ? 'Connecting...' : 'Extract DDL'}
        </button>
      </form>
    </div>
  );
...
$$;`,
  ...
  const handleDdlExtracted = (extractedDdl) => {
    setDdl(extractedDdl);
    setData(null);
    setError('');
    setSuccess('DDL extracted from database successfully.');
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        <DatabaseConnectionForm onDdlExtracted={handleDdlExtracted} />
